...
const Setting = () => {
  ...
  const [pdfToggle, setPdfToggle] = useState(() => localStorage.getItem('personalizedPdf') !== 'false')

  const togglePdf = () => {
    const next = !pdfToggle
    setPdfToggle(next)
    localStorage.setItem('personalizedPdf', String(next))
    toast.success(next ? 'Personalized PDF enabled' : 'Personalized PDF disabled')
  }
      ...
      <div className='mx-auto max-w-6xl'>
        ...
        <div className='grid grid-cols-1 lg:grid-cols-12 gap-8'>
          ...
          <div className='lg:col-span-8 space-y-6'>
            ...
            <div className='rounded-[32px] border border-slate-200 bg-white p-6 shadow-[0_15px_40px_-20px_rgba(15,23,42,0.1)]'>
              <div className='flex items-center gap-3 border-b border-slate-100 pb-5 mb-5'>
                <div className='h-9 w-9 rounded-xl bg-violet-50 flex items-center justify-center border border-violet-100'>
                  <svg className='h-5 w-5 text-violet-600' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
                    <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' />
                  </svg>
                </div>
                <div>
                  <h3 className='text-sm font-bold text-slate-800 tracking-tight'>PDF Settings</h3>
                  <p className='text-[10px] text-slate-400 font-semibold tracking-wider uppercase mt-0.5'>Customize downloaded documents</p>
                </div>
              </div>

              <div className='flex items-start justify-between gap-4 rounded-2xl bg-gradient-to-r from-violet-50 to-purple-50 p-4 border border-violet-100'>
                <div className='flex items-start gap-3'>
                  <div className='h-10 w-10 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shrink-0 shadow-md shadow-violet-500/20 mt-0.5'>
                    <svg className='h-5 w-5 text-white' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
                      <path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z' />
                    </svg>
                  </div>
                  <div>
                    <p className='text-sm font-bold text-slate-900'>Personalized PDF Cover Page</p>
                    <p className='text-[10px] font-semibold text-slate-500 mt-1 leading-relaxed max-w-xs'>
                      When enabled, a branded cover page with your advisor details, business logo, and policy summary is prepended to every downloaded Insurance PDF.
                    </p>
                    <div className='mt-2 flex flex-wrap gap-1.5'>
                      {['Advisor Info', 'Policy Summary', 'Contact Details'].map(tag => (
                        <span key={tag} className='text-[9px] font-bold bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full'>{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
                <button
                  id='personalized-pdf-toggle'
                  onClick={togglePdf}
                  className={`relative shrink-0 w-12 h-6 rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-violet-500/40 ${pdfToggle ? 'bg-violet-600' : 'bg-slate-200'}`}
                  aria-label='Toggle personalized PDF'
                >
                  <span className={`absolute top-0.5 left-0.5 h-5 w-5 rounded-full bg-white shadow transition-transform duration-300 ${pdfToggle ? 'translate-x-6' : 'translate-x-0'}`} />
                </button>
              </div>

              {pdfToggle && (
                <div className='mt-3 flex items-center gap-2 rounded-xl bg-violet-50 border border-violet-100 px-3 py-2'>
                  <svg className='h-3.5 w-3.5 text-violet-500 shrink-0' fill='currentColor' viewBox='0 0 20 20'>
                    <path fillRule='evenodd' d='M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z' clipRule='evenodd' />
                  </svg>
                  <p className='text-[10px] font-semibold text-violet-700'>Cover page uses your business name, logo, and contact info from your profile.</p>
                </div>
              )}
            </div>
